...
public class PedidoCompletadoListener {
    ...
    public PedidoCompletadoListener(ConnectionFactory connectionFactory) {
    ...
    public void gestionarPedidoCompletado(PedidoCompletadoEvent event) {
        ...
        try {
            ...
            try (JMSContext jms = connectionFactory.createContext()) {
                jms.createProducer().send(jms.createQueue(JmsConfig.COLA_WEB_PEDIDOS), xml);
            }
            logger.info("Pedido enviado a cola {}: {}", JmsConfig.COLA_WEB_PEDIDOS, xml);
